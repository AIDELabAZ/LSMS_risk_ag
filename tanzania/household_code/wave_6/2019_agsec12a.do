* Project: WB Weather
* Created on: March 2024
* Created by: reece
* Edited on: oct 20 2024
* Edited by: reece
* Stata v.18

* does
	* cleans tza access to extension variable
	
* assumes
	* access to all raw data
	* mdesc.ado
	* cleaned hh_seca.dta

* TO DO:
	* 
* **********************************************************************
**#0 - setup
* **********************************************************************

* define paths
	global root 	"$data/raw_lsms_data/tanzania/wave_6/raw"
	global export 	"$data/lsms_risk_ag_data/refined_data/tanzania/wave_6"
	global logout 	"$data/lsms_risk_ag_data/refined_data/tanzania/logs"

* open log 
	cap log close 
	log using "$logout/wv6_AGSEC12A", append

	
* ***********************************************************************
**#1 - prepare TZA 2020 (Wave 7) - Agriculture Section 4A 
* ***********************************************************************

* load data
	use 		"$root/ag_sec_12a", clear

	drop		ag12a_02 ag12a_03_1 ag12a_03_2 ag12a_03_3 ag12a_03_4 ag12a_04 ag12a_05 ag12a_06 ag12a_07
	drop 		interview__key ag12a_01_3 ag12a_01_4 ag12a_01_5 ag12a_01_6 ag12a_01_7 ag12a_01_8
	
	replace ag12a_01_1 = 0 if ag12a_01_1 == 2
	replace ag12a_01_2 = 0 if ag12a_01_2 == 2
	
	 reshape wide ag12a_01_1 ag12a_01_2, i(sdd_hhid) j(sourceid)
	 egen access = rowtotal (ag12a_01_11 ag12a_01_21 ag12a_01_12 ag12a_01_22 ag12a_01_13 ag12a_01_23 ag12a_01_14 ag12a_01_24 ag12a_01_15 ag12a_01_25)
	 
	 replace access = 1 if access > 0
	 
* must merge in regional identifiers from 2020_HHSECA
	merge		1:1 sdd_hhid using "$export/HH_SECA"
	tab			_merge		 
	* only 60 percent merging

	keep sdd_hhid region district ward ea access 
	 
* prepare for export
	isid			sdd_hhid region ward ea district
	compress
	describe
	summarize 
	save 			"$export/2019_AGSEC12A.dta", replace

* close the log
	log	close

/* END */

	 
	 
	 
