...
* does
	* creates output tables for Nigeria results in .txt file
...
* open log to capture overall console messages if desired
	cap log close
...
********************************************************************************
**#3.1 - loop through rainfall, lag, and shocks — save to file
********************************************************************************

* rainfall, lagged, and shock variables
...
* delete old output if it exists
	cap erase "$export/model_comparisons.txt"
			...
			* loop over matching shocks
			foreach s in `shock' {
				if substr("`v'", 7, 1) == substr("`s'", 7, 1) {
					...
					* Save to file
					esttab model1 model2 using "$export/model_comparisons_nga.txt", ///
						append se star(* 0.1 ** 0.05 *** 0.01) ///
						label compress nomtitle nogaps ///
						title("Rain: `v' | Lag: `t' | Shock: `s'")
