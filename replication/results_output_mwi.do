...
* does
	* creates output tables for Malawi results in .txt file
...
********************************************************************************
	...
	cap erase 		"$export/model_comparisons_mwi.txt"
			...
			foreach s in `shock' {
				if substr("`v'", 7, 1) == substr("`s'", 7, 1) {
					...
					* Save to results file
					esttab model1 model2 using "$export/model_comparisons_mwi.txt", ///
						append se star(* 0.1 ** 0.05 *** 0.01) ///
						label compress nomtitle nogaps ///
						title("Malawi | Rain: `v' | Lag: `t' | Shock: `s'")
