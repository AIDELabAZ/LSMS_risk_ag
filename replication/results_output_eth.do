...
********************************************************************************
	...
	program define reg3_safe_mod1, rclass
		...
		capture noisily reg3 (mu1_s mu2_s mu3_s) (mu1_f mu2_f mu3_f), ///
	...
	* NOTE:
	* bootstrap can choke when passing program arguments. We pass the current
	* shock variable name via a global macro $SHOCKVAR instead.
	capture program drop reg3_safe_mod2
	program define reg3_safe_mod2, rclass
		version 18

		local shockvar "$SHOCKVAR"
		if "`shockvar'" == "" {
			di as error "SHOCKVAR global is empty"
			exit 198
		}
...
********************************************************************************
				...
				*** IV RELEVANCE DIAGNOSTICS (FIRST MOMENT ONLY) ***
				...
				* coefficients needed for derivatives
				matrix 	a = e(b)
				scalar 	b1_f  = a[1,1]
				scalar 	b1_f2 = a[1,2]
				scalar 	b1_s  = a[1,3]
				...
				*** WRITE DIAGNOSTICS ONCE PER (v,t) ***
...
********************************************************************************
	...
	* clean up global used for Model 2 bootstrap calls
	macro drop SHOCKVAR
